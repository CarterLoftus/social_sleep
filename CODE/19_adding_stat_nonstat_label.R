...
manual_ident <- fread( "DATA\\identification_in_thermal\\final_id_and_focal.csv" )
...
files <- list.files( path = "DATA/thermal_tracks/smooth_tracks/corrected/" )
...
for( ni in 1:length( nights ) ){
  ...
  pre_id_tracks <- readRDS( paste0( "DATA/thermal_tracks/smooth_tracks/corrected/", night, "_smooth_tracks.rds" ) )
...
### this comes from CODE/15_prepping_therm_focals.R script
...
# how will did our simple speed threshold or 0.25 meters/second do?
...
#### now use the model to add stationary vs. non-stationary information to the tracks

## from here down needs to be run for both corrected and uncorrected tracks

nonstat_model <- readRDS( 'DATA/thermal_tracks/stat_nonstat_ml_model.rds' )

input_data <- 'uncorrected' ## 'corrected' or 'uncorrected'. Make sure you have run the script from the top if running for corrected tracks
...
if( input_data == 'corrected' ){
  ...
  files_to_read <- list.files( path = paste0( "DATA/thermal_tracks/smooth_tracks/", input_data ), full.names = T )
...
for( final_track in final_tracks_list ){
  ...
  saveRDS( final_track, paste0( "DATA/thermal_tracks/smooth_tracks/", input_data, '/', gsub( '-', '', night ) , "_smooth_tracks.rds" ) )
