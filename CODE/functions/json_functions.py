...
def create_train_val_split(json_file, fraction_val):
    """
        Args:
    ...
    with open(os.path.join(save_folder, "train.json"), "w") as write_file:
        json.dump(train_dict, write_file, indent=4, separators=(',', ': '))

    with open(os.path.join(save_folder, "val.json"), "w") as write_file:
