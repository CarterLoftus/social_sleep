...
class TrackCorrectionGUI():
    ...
    def __init__(self, tracks_path, frame_files, factor,
        ...
        self.trackcount = 1
        self.framecount = 0
        temp_frame = self.focal_tracks[0]['first_frame'] #to get good frame
        ...
        if not self.hide:
            ...
            if rel_frame_new_track > 0:
                ...
                if self.listoftracks[self.trackcount]['first_frame'] + len(self.listoftracks[self.trackcount]['track']) > self.framecount:
                    ...
                    self.draw_points(image, [current_point*self.factor], 50,255,0, point_size*2)
...
#         new_pic = cv2.resize(image, (self.w, self.h))
    ...
    def detect_keys(self, key):
        ...
        if key == ord('d'):
            ...
            self.change_frame_function(-500)
        #elif key == 2 or key == 81: #left key = move frame back
        elif key == ord('6'):
            self.change_frame_function(500)
        elif key == 58:
