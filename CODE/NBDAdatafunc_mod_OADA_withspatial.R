setClass(
  ...
  representation(
    ...
    orderAcq = "vector",
    timeAcq = "vector",
    durationAcq = "vector", # SW: adding duration
    endTime = "numeric",
    updateTimes = "vector",
    ties = "vector",
    trueTies = "list",
    demons = "vector",
    demons.duration = "vector", #SW: how long demonstrators were awake for until going back to sleep
    weights = "vector",
    statusMatrix = "matrix",
    ...
    TADAtime2 = "vector",
    status = "vector",
    presentInDiffusion = "vector",
    assMatrixIndex = "vector",
    asocialTreatment = "character",
    stMetric = "matrix",
    asocILVdata = "matrix",
    intILVdata = "matrix",
    multiILVdata = "matrix",
    offsetCorrection = "matrix",
    randomEffectdata = "matrix",
    type ="character"
  )
)
...
setMethod("initialize",
          ...
          function (.Object,
                    ...
                    int_ilv = "ILVabsent",
                    multi_ilv = "ILVabsent",
                    random_effects = "REabsent",
                    orderAcq,
                    timeAcq,
                    durationAcq, #SW: add duration
                    endTime,
                    ties = NULL,
                    trueTies = list(NULL),
                    id = NA,
                    ...
                    ...)
          {
            ...
            if ( type=="OADA") {
              ...
              nAcq <-
                ifelse(any(is.na(orderAcq)), 0, length(orderAcq)) # number of acquisition events EXCLUDING demonstrators.
              time1 <- vector()
              # time period index: time start
              time2 <- vector()
              # time period index: time end
              event.id.temp <-
                vector()
              # vector to indicate the event number (this is essentially indexing the naive individuals before each event)
              
              #If no asoc variable is provided, set a single column of zeroes
              if (asoc_ilv[1] == "ILVabsent" |
                  int_ilv[1] == "ILVabsent" | multi_ilv[1] == "ILVabsent") {
                ILVabsent <-
                  matrix(
                    data = rep(0, dim(assMatrix)[1]),
                    nrow = dim(assMatrix)[1],
                    byrow = F
                  )
              }
              ...
              totalMetric <-
                vector() # total association of the individual that DID learn at an acquisition event, with all other individuals
              learnMetric <-
                vector()
              # total associations of the individual that DID learn at an acquisition event, with all other individuals that have already learned
              
              status <-
                presentInDiffusion <-
                vector()
              # set up the status vector and presentInDiffusion vector
              ...
              asoc_ilv.dim <-
                dim(eval(as.name(asoc_ilv[1])))[1] # specify the dimensions of assoc.array
              int_ilv.dim <-
                dim(eval(as.name(int_ilv[1])))[1] # specify the dimensions of assoc.array
              multi_ilv.dim <-
                dim(eval(as.name(multi_ilv[1])))[1] # specify the dimensions of assoc.array
              random_effects.dim <-
                dim(eval(as.name(random_effects[1])))[1] # specify the dimensions of assoc.array
              ...
              if (asocialTreatment == "constant") {
                ...
                dimnames(asoc_ilv.array) <-
                  list(NULL, NULL, asoc_ilv)
                int_ilv.array <-
                  array(dim = c(int_ilv.dim, 1, length(int_ilv)))
                dimnames(int_ilv.array) <- list(NULL, NULL, int_ilv)
                multi_ilv.array <-
                  array(dim = c(multi_ilv.dim, 1, length(multi_ilv)))
                dimnames(multi_ilv.array) <-
                  list(NULL, NULL, multi_ilv)
                random_effects.array <-
                  array(dim = c(random_effects.dim, 1, length(random_effects)))
                dimnames(random_effects.array) <-
                  list(NULL, NULL, random_effects)
                
              } else {
                if (asocialTreatment == "timevarying") {
                  asoc_ilv.array <- array(dim = c(asoc_ilv.dim, nAcq, length(asoc_ilv)))
                  dimnames(asoc_ilv.array) <-
                    list(NULL, c(paste("time", c(1:nAcq), sep = "")), asoc_ilv)
                  int_ilv.array <-
                    array(dim = c(int_ilv.dim, nAcq, length(int_ilv)))
                  dimnames(int_ilv.array) <-
                    list(NULL, c(paste("time", c(1:nAcq), sep = "")), int_ilv)
                  multi_ilv.array <-
                    array(dim = c(multi_ilv.dim, nAcq, length(multi_ilv)))
                  dimnames(multi_ilv.array) <-
                    list(NULL, c(paste("time", c(1:nAcq), sep = "")), multi_ilv)
                  random_effects.array <-
                    array(dim = c(random_effects.dim, nAcq, length(random_effects)))
                  dimnames(random_effects.array) <-
                    list(NULL, c(paste("time", c(1:nAcq), sep = "")), random_effects)
                }
              }
              
              # generate a matrix that contains the status of each individual at each acquisition event
              statusMatrix <-
                matrix(0, nrow = dim(assMatrix)[2], ncol = 1 + nAcq)  # a matrix with as many rows as indivs and as many columns as acquisition events PLUS one for the demonstrators
              # create a list vector to hold the index of naive individuals after each acquisition event
              naive.id <-
                naive.id.names <- vector(mode = "list", length = nAcq)
              ...
              if (is.null(demons)) {
                ...
                #for (i in 1:(1 + nAcq)) { # SW: adjust
                demons.position <- which(demons==1)
                for(i in demons.position){
                  statusMatrix[i, 1:(1+as.numeric(demons.duration[i]))] <- 1 #fill the first column with 1s for demonstrators + how many extra minutes they were awake for
                }
                #  statusMatrix[demons, i] <- demons
                #}
              }
              
              availabilityMatrix <-
                statusMatrix # if there are ties the statusMatrix and the availabilityMatrix will differ (the latter gives who is available to be learned *from*). we create it as identical and modify it accordingly below
              ...
              if (is.null(presenceMatrix)) {
                ...
                presenceMatrix <-
                  cbind(presenceMatrix[, 1], presenceMatrix)
              }
              ...
              asocILVdata.naive <-
                intILVdata.naive <-
                multiILVdata.naive <-
                randomEffectdata.naive <-
                vector() # this will hold the individual level variables for the naive individuals
              ...
              if (nAcq == 0) {
                learnAsoc <-
                  learnInt <- multiInt <- naive.id <- time1 <- time2 <- stMetric <- NA
                ...
                learnAsoc <-
                  matrix(nrow = length(asoc_ilv), ncol = nAcq) # a matrix with as many rows as individuals and as many columns as acquisition events
                dimnames(learnAsoc) <-
                  list(NULL, c(paste(
                    "id", c(orderAcq), "time", c(1:nAcq), sep = ""
                  )))
                
                learnInt <-
                  matrix(nrow = length(int_ilv), ncol = nAcq) # a matrix with as many rows as individuals and as many columns as acquisition events
                dimnames(learnInt) <-
                  list(NULL, c(paste(
                    "id", c(orderAcq), "time", c(1:nAcq), sep = ""
                  )))
                ...
                learnMulti <-
                  matrix(nrow = length(multi_ilv), ncol = nAcq) # a matrix with as many rows as individuals and as many columns as acquisition events
                dimnames(learnMulti) <-
                  list(NULL, c(paste(
                    "id", c(orderAcq), "time", c(1:nAcq), sep = ""
                  )))
                ...
                learnRE <-
                  matrix(nrow = length(random_effects), ncol = nAcq) # a matrix with as many rows as individuals and as many columns as acquisition events
                dimnames(learnRE) <-
                  list(NULL, c(paste(
                    "id", c(orderAcq), "time", c(1:nAcq), sep = ""
                  )))
              ...
              for (a in 1:length(asoc_ilv)) {
                # Loop through asocial variables - a loop
                asoc_ilv.array[, , a] <-
                  eval(as.name(asoc_ilv[a])) # evaluate each one in turn
              }
              for (a in 1:length(int_ilv)) {
                # Loop through asocial variables - a loop
                int_ilv.array[, , a] <-
                  eval(as.name(int_ilv[a])) # evaluate each one in turn
              }
              for (a in 1:length(multi_ilv)) {
                # Loop through asocial variables - a loop
                multi_ilv.array[, , a] <-
                  eval(as.name(multi_ilv[a])) # evaluate each one in turn
              }
              for (a in 1:length(random_effects)) {
                # Loop through asocial variables - a loop
                random_effects.array[, , a] <-
                  eval(as.name(random_effects[a])) # evaluate each one in turn
              }
              
              if (nAcq != 0) {
                for (i in 1:nAcq) {
                  # Loop through acquisition events - i loop
                  
                  k <-
                    ifelse(asocialTreatment == "constant", 1, i) # this makes sure you are treating ILVs correctly if they are constant and if they are time-varying
                  
                  for (a in 1:length(asoc_ilv)) {
                    # Loop through asocial variables - a loop
                    learnAsoc[a, i] <-
                      asoc_ilv.array[orderAcq[i], k, a] # fill the matrix with the rows of the asoc matrix that correspond to the individual that learned at each acquisition event. each column is an individual, each row is a type of variable
                  }
                  for (a in 1:length(int_ilv)) {
                    # Loop through asocial variables - a loop
                    learnInt[a, i] <-
                      int_ilv.array[orderAcq[i], k, a] # fill the matrix with the rows of the asoc matrix that correspond to the individual that learned at each acquisition event. each column is an individual, each row is a type of variable
                  }
                  for (a in 1:length(multi_ilv)) {
                    # Loop through asocial variables - a loop
                    learnMulti[a, i] <-
                      multi_ilv.array[orderAcq[i], k, a] # fill the matrix with the rows of the asoc matrix that correspond to the individual that learned at each acquisition event. each column is an individual, each row is a type of variable
                  }
                  for (a in 1:length(random_effects)) {
                    # Loop through asocial variables - a loop
                    learnRE[a, i] <-
                      random_effects.array[orderAcq[i], k, a] # fill the matrix with the rows of the asoc matrix that correspond to the individual that learned at each acquisition event. each column is an individual, each row is a type of variable
                  }
                  # SW: we use the same hack as for TADA
                  # SW: using the duration and time of the event, we extract at which events the individual was knowledgeable
                  # SW: once it 'unlearns' the behaviour, its status goes back to 0, so it can 'relearn'
                  
                  events.informed <- which(timeAcq[i]<= timeAcq & timeAcq<=(timeAcq[i]+durationAcq[i]))
                  statusMatrix[orderAcq[i], events.informed+1] <- 1
                  
                  #statusMatrix[orderAcq[i], c((i + 1):(nAcq + 1))] <- 1
                  # give the individuals that acquired the trait a status of 1 and carry skilled status (1) through to all following acquisition events
                  ...
                  availabilityMatrix <-
                    availabilityMatrix * presenceMatrix
                  
                  
                  # SW: here, we need to modify the code to extract which individuals were naive BEFORE each acquisition event, as in the original code, individuals are considered staying knowledgeable onc they have acquired a behaviour. 
                  
                  # extract the time of event i
                  time.of.i <- timeAcq[i]
                  # we need to extract who was awake at time.of.i minus 1 minute
                  naive.at.i <- NULL
                  for(agent in 1:nrow(statusMatrix)){
                    awake.agent <- NULL
                    awake.at.which.minutes.agent <- timeAcq[which(orderAcq==agent)]
                    if(demons[agent]==1){
                      awake.at.which.minutes.agent <- c(0:(0+demons.duration[agent]), awake.at.which.minutes.agent)
                    }
                    if(length(awake.at.which.minutes.agent>0)){
                      for(t in 1:length(awake.at.which.minutes.agent)){
                        awake <-awake.at.which.minutes.agent[t]:(awake.at.which.minutes.agent[t]+durationAcq[which(orderAcq==agent)][t])
                        awake.agent <- c(awake.agent, awake) 
                      }
                    }
                    if(!((time.of.i-1) %in% awake.agent)){
                      naive.id[[i]] <- c(naive.id[[i]], agent)
                    }
                  } # agent
              ...
              if (is.na(id[1])) {
                id <-
                  paste(label, c(unlist(naive.id)), sep = "_")
              } # id of naive individuals before each acquisition event, including demonstrators
              
              # naive <-
              #   dim(assMatrix)[1] - apply(statusMatrix, 2, sum) # number of naive individuals remaining after each acq event
              # SW - need to ajust - not take info from status matrix but from naive.id list
              
              naive <- unlist(lapply(naive.id,length))  
              
              # work out the number of association matrices provided and set up stMetric matrix accordingly
              stMetric <-
                matrix(data = 0,
                       nrow = length(id),
                       ncol = dim(assMatrix)[3])
              dimnames(stMetric) <-
                list(NULL, paste("stMetric", c(1:dim(assMatrix)[3]), sep = ""))
              ...
              if (nAcq != 0) {
                for (event in 1:nAcq) {
                  ...
                  if (is.na(event.id[1])) {
                    event.id.temp <-
                      c(event.id.temp, rep(event, each = length(naive.id[[event]])))
                  }
                } # closes for loop through events
                
                if (is.na(event.id[1])) {
                  event.id <- paste(label, event.id.temp, sep = "_")
                }
                
                for (event in 1:nAcq) {
                  # event indexes the number of the acquisition event
                  ...
                  assMatrix <-
                    array(assMatrixTV[, , , assMatrixIndex[event]], dim = dim(assMatrixTV)[1:3])
                  
                  learner <-
                    orderAcq[event] # learner is individual id of the animal that learned AT an event
                  nonlearners <-
                    naive.id[[event]] # nonlearners are individual id of the animals that were naive BEFORE an event
                  status <-
                    c(status, statusMatrix[unlist(naive.id[[event]]), event + 1])
                  presentInDiffusion <-
                    c(presentInDiffusion, presenceMatrix[unlist(naive.id[[event]]), event +
                                                           1])
                  
                  temp.stMetric <-
                    vector() # reset this before the metrics for each event are calculated
                  ...
                  for (nonlearner in nonlearners) {
                    ...
                    m1 <-
                      matrix(
                        data = assMatrix[nonlearner, , ],
                        nrow = dim(assMatrix)[3],
                        byrow = T
                      ) # matrix1
                    m2 <-
                      (weights * availabilityMatrix[, event]) * t(m1) # matrix2
                    v1 <-
                      apply(X = m2,
                            MARGIN = 2,
                            FUN = sum) # vector1 of rowsums
                    temp.stMetric <- rbind(temp.stMetric, v1)
                    
                  } # closes nonlearner loop for MATRIX stMetric
                  rownames(temp.stMetric) <- NULL
                  stMetric[which(time2 == event), ] <- temp.stMetric
                  ...
                  if (asoc_ilv[1] == "ILVabsent") {
                    ...
                    if (asocialTreatment == "constant") {
                      ilv1 <-
                        matrix(asoc_ilv.array[nonlearners, 1, ], nrow = length(nonlearners))
                    } else{
                      ilv1 <-
                        matrix(asoc_ilv.array[nonlearners, event, ], nrow = length(nonlearners))
                    }
                  ...
                  if (int_ilv[1] == "ILVabsent") {
                    ...
                    if (asocialTreatment == "constant") {
                      intilv1 <-
                        matrix(int_ilv.array[nonlearners, 1, ], nrow = length(nonlearners))
                    } else{
                      intilv1 <-
                        matrix(int_ilv.array[nonlearners, event, ], nrow = length(nonlearners))
                    }
                  }# this makes sure the right column out of the asoc.array is used
                  ...
                  if (multi_ilv[1] == "ILVabsent") {
                    ...
                    if (asocialTreatment == "constant") {
                      multiilv1 <-
                        matrix(multi_ilv.array[nonlearners, 1, ], nrow = length(nonlearners))
                    } else{
                      multiilv1 <-
                        matrix(multi_ilv.array[nonlearners, event, ], nrow = length(nonlearners))
                    }
                  }# this makes sure the right column out of the asoc.array is used
                  ...
                  if (random_effects[1] == "REabsent") {
                    ...
                    if (asocialTreatment == "constant") {
                      randomeffect1 <-
                        matrix(random_effects.array[nonlearners, 1, ], nrow = length(nonlearners))
                    } else{
                      randomeffect1 <-
                        matrix(random_effects.array[nonlearners, event, ], nrow = length(nonlearners))
                    }
                  }# this makes sure the right column out of the asoc.array is used
                  ...
                  intILVdata.naive <-
                    rbind(intILVdata.naive, intilv1)
                  if (int_ilv[1] == "ILVabsent") {
                  ...
                  multiILVdata.naive <-
                    rbind(multiILVdata.naive, multiilv1)
                  if (multi_ilv[1] == "ILVabsent") {
                  ...
                  randomEffectdata.naive <-
                    rbind(randomEffectdata.naive, randomeffect1)
                  if (random_effects[1] == "REabsent") {
                    attr(randomEffectdata.naive, "dimnames") <- list(NULL, "REabsent")
                  } else{
                    attr(randomEffectdata.naive, "dimnames") <-
                      list(NULL, random_effects)
                  }
                  
                } # closes event loop
              } else {
                # closes if(nAcq!=0) statement
                asocILVdata.naive <-
                  matrix(
                    data = rep(0, length(asoc_ilv)),
                    nrow = 1,
                    ncol = length(asoc_ilv)
                  )
                intILVdata.naive <-
                  matrix(
                    data = rep(0, length(int_ilv)),
                    nrow = 1,
                    ncol = length(int_ilv)
                  )
                multiILVdata.naive <-
                  matrix(
                    data = rep(0, length(multi_ilv)),
                    nrow = 1,
                    ncol = length(multi_ilv)
                  )
                randomEffectdata.naive <-
                  matrix(
                    data = rep(0, length(random_effects)),
                    nrow = 1,
                    ncol = length(random_effects)
                  )
                
                attr(asocILVdata.naive, "dimnames") <-
                  list(NULL, asoc_ilv)
                attr(multiILVdata.naive, "dimnames") <-
                  list(NULL, int_ilv)
                attr(intILVdata.naive, "dimnames") <-
                  list(NULL, multi_ilv)
                attr(randomEffectdata.naive, "dimnames") <-
                  list(NULL, random_effects)
              } # closes else
              ...
              if (is.null(offsetCorrection))
                offsetCorrection <-
                cbind(rep(0, dim(asocILVdata.naive)[1]), rep(0, dim(asocILVdata.naive)[1]), rep(0, dim(asocILVdata.naive)[1]), rep(0, dim(asocILVdata.naive)[1]))
              
              dimnames(offsetCorrection)[2] <-
                list(
                  c(
                    "SocialOffsetCorr",
                    "AsocialILVOffsetCorr",
                    "InteractionOffsetCorr",
                    "MultiplicativeILVOffsetCorr"
                  )
                )
              ...
              callNextMethod(
                ...
                timeAcq = timeAcq,
                endTime = endTime,
                durationAcq = durationAcq,
                updateTimes = NA,
                ties = ties,
                trueTies = trueTies,
                demons = demons,
                #    demons.duration = demons.duration, # SW add
                weights = weights,
                ...
                time2 = time2,
                status = status,
                presentInDiffusion = presentInDiffusion,
                presenceMatrix = presenceMatrix ,
                asocialTreatment = asocialTreatment,
                stMetric = stMetric,
                asocILVdata = asocILVdata.naive,
                intILVdata = intILVdata.naive,
                multiILVdata = multiILVdata.naive,
                randomEffectdata = randomEffectdata.naive,
            ...
            #     if (asocialTreatment == "timevarying") {
            #       asoc_ilv.array <-
            #         array(dim = c(asoc_ilv.dim, (nAcq + 1), length(asoc_ilv)))
            #       dimnames(asoc_ilv.array) <-
            #         list(NULL, c(paste("time", c(1:(
            #           nAcq + 1
            #         )), sep = "")), asoc_ilv)
            #       int_ilv.array <-
            #         array(dim = c(int_ilv.dim, (nAcq + 1), length(int_ilv)))
            #       dimnames(int_ilv.array) <-
            #         list(NULL, c(paste("time", c(1:(
            #           nAcq + 1
            #         )), sep = "")), int_ilv)
            #       multi_ilv.array <-
            #         array(dim = c(multi_ilv.dim, (nAcq + 1), length(multi_ilv)))
            #       dimnames(multi_ilv.array) <-
            #         list(NULL, c(paste("time", c(1:(
            #           nAcq + 1
            #         )), sep = "")), multi_ilv)
            #       random_effects.array <-
            #         array(dim = c(random_effects.dim, (nAcq + 1), length(random_effects)))
            #       dimnames(random_effects.array) <-
            #         list(NULL, c(paste("time", c(1:(
            #           nAcq + 1
            #         )), sep = "")), random_effects)
            ...
            #   } # closes else
            #   
            #   if (dim(eval(as.name(asoc_ilv[1])))[2] == (nAcq + 1) |
            #       dim(eval(as.name(asoc_ilv[1])))[2] == 1) {
            #     for (a in 1:length(asoc_ilv)) {
            #       # Loop through asocial variables - a loop
            #       asoc_ilv.array[, , a] <-
            #         eval(as.name(asoc_ilv[a])) # evaluate each one in turn
            ...
            #     for (a in 1:length(asoc_ilv)) {
            #       # Loop through asocial variables - a loop
            #       asoc_ilv.array[, 1:nAcq, a] <-
            #         eval(as.name(asoc_ilv[a])) # evaluate each one in turn
            #       asoc_ilv.array[, nAcq + 1, a] <-
            #         asoc_ilv.array[, nAcq, a]
            #     }
            #   }
            #   
            #   
            #   if (dim(eval(as.name(int_ilv[1])))[2] == (nAcq + 1) |
            #       dim(eval(as.name(int_ilv[1])))[2] == 1) {
            #     for (a in 1:length(int_ilv)) {
            #       # Loop through asocial variables - a loop
            #       int_ilv.array[, , a] <-
            #         eval(as.name(int_ilv[a])) # evaluate each one in turn
            ...
            #     for (a in 1:length(int_ilv)) {
            #       # Loop through asocial variables - a loop
            #       int_ilv.array[, 1:nAcq, a] <-
            #         eval(as.name(int_ilv[a])) # evaluate each one in turn
            #       int_ilv.array[, nAcq + 1, a] <-
            #         int_ilv.array[, nAcq, a]
            #     }
            #   }
            #   
            #   if (dim(eval(as.name(multi_ilv[1])))[2] == (nAcq + 1) |
            #       dim(eval(as.name(multi_ilv[1])))[2] == 1) {
            #     for (a in 1:length(multi_ilv)) {
            #       # Loop through asocial variables - a loop
            #       multi_ilv.array[, , a] <-
            #         eval(as.name(multi_ilv[a])) # evaluate each one in turn
            ...
            #     for (a in 1:length(multi_ilv)) {
            #       # Loop through asocial variables - a loop
            #       multi_ilv.array[, 1:nAcq, a] <-
            #         eval(as.name(multi_ilv[a])) # evaluate each one in turn
            #       multi_ilv.array[, nAcq + 1, a] <-
            #         multi_ilv.array[, nAcq, a]
            #     }
            #   }
            #   
            #   if (dim(eval(as.name(random_effects[1])))[2] == (nAcq + 1) |
            #       dim(eval(as.name(random_effects[1])))[2] == 1) {
            #     for (a in 1:length(random_effects)) {
            #       # Loop through asocial variables - a loop
            #       random_effects.array[, , a] <-
            #         eval(as.name(random_effects)) # evaluate each one in turn
            #     }
            #   } else{
            #     # If no values are provided for the final period to endTime, the values are assumed to be the same as for the final event
            #     for (a in 1:length(random_effects)) {
            #       # Loop through asocial variables - a loop
            #       random_effects.array[, 1:nAcq, a] <-
            #         eval(as.name(random_effects[a])) # evaluate each one in turn
            #       random_effects.array[, nAcq + 1, a] <-
            #         random_effects.array[, nAcq, a]
            #     }
            #   }
            #   
            #   
            ...
            #       
            #       
            #       # SW: here, we give the individual that acquired the trait a status of 1
            #       # statusMatrix[orderAcq[i], c((i + 1):(nAcq + 1))] <-  0 # give the individuals that acquired the trait a status of 1 and carry skilled status (1) through to all following acquisition events
            #       
            #       # SW: but we modify the original code so that we can add the duration which it was knowledgeable for
            #       events.informed <- which(timeAcq[i]<= timeAcq & timeAcq<=(timeAcq[i]+durationAcq[i]))
            #       statusMatrix[orderAcq[i], events.informed+1] <- 1
            #       
            #       
            ...
            #     
            #       # naive.id[[i]] <-
            #       #   which(statusMatrix[, i] == 0) # index for naive individuals before the ith acquisition event
            #       # 
            #     
            #     
            #   } # closes the i loop - nAcq (k is i or 1)
            ...
            #   } # id of naive individuals before each acquisition event, including demonstrators
            #   
            #   # SW: this could be useful to add to NBDA data object to see at one glance how many were 
            #   # asleep/awake at each time step
            #   # naive <-
            #   #   dim(assMatrix)[1] - apply(statusMatrix, 2, sum) # number of naive individuals remaining after each acq event (last event will be the end of the diffusion for TADA data with incomplete diffusion)
            #   naive <- unlist(lapply(naive.id,length))  
            #   
            #   # work out the number of association matrices provided and set up stMetric matrix accordingly
            #   # stMetric <-
            ...
            #   #          ncol = dim(assMatrix)[3])
            #   
            #   #SW: copy from below: stMetric is the total assoc of the individuals that did NOT learn by that acquisition event, with all other already-informed individuals
            #   # adjust to unique(id), as in this case, individuals could be naive more than once (after already having learned)
            #   stMetric <-
            #     matrix(data = 0,
            #            nrow = length(id),
            #            ncol = dim(assMatrix)[3])
            ...
            #   
            #   
            #   ## SW: loop through all events starting here
            #   for (event in 1:(nAcq + 1)) {
            #     # event indexes the number of the acquisition event- increased by 1 for TADA to allow for the endTime period
            #     
            #     #Take the appropriate association matrix from the (weighted) time varying association matrix,
            #     #as determined for that event by the assMatrixIndex vector
            #     if ((length(assMatrixIndex) == nAcq) &
            #         (event == (nAcq + 1))) {
            #       #If no separate assMatrix is specified for the end period it is assumed to be the same as for the final acquisition event
            #       assMatrix <-
            #         array(assMatrixTV[, , , assMatrixIndex[nAcq]], dim = dim(assMatrixTV)[1:3])
            #     } else{
            #       assMatrix <-
            #         array(assMatrixTV[, , , assMatrixIndex[event]], dim = dim(assMatrixTV)[1:3])
            #     }
            ...
            #       
            #       status <-
            #         c(status, statusMatrix[unlist(naive.id[[event]]), min(nAcq + 1, event +
            #                                                                 1)])
            #       presentInDiffusion <-
            #         c(presentInDiffusion, presenceMatrix[unlist(naive.id[[event]]), min(nAcq +
            #                                                                               1, event + 1)])
            #       
            #       temp.stMetric <-
            #         vector() # reset this before the metrics for each event are calculated
...
nbdaData <-
  function(label,
    ...
    if (!is.null(asoc) & asoc_ilv[1] == "ILVabsent")
      asoc_ilv <- asoc
